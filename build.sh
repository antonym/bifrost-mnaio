...
ansible-playbook -vv \
                 ...
                 -e default_image=${DEFAULT_IMAGE:-"centos-7-amd64"} \
                 ...
                 -e deployment_vm_server_ram=${DEPLOYMENT_VM_SERVER_RAM:-"16384"} \
                 -e swift_vm_server_ram=${SWIFT_VM_SERVER_RAM:-"4096"} \
                 -e director_vm_server_vcpus=${DEPLOYMENT_VM_SERVER_VCPUS:-"8"} \
                 -e ipxe_kernel_url=${IPXE_KERNEL_URL:-"http://boot.ipxe.org/ipxe.lkrn"} \
                 -e redhat_iso_url=${REDHAT_ISO_URL:-""} \
                 -e redhat_base_url=${REDHAT_BASE_URL:-"https://mirrors.edge.kernel.org/centos/7/"} \
